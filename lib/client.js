window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const QueueSteerActions = (props) => {
			...
			/**
			 * Escape: stop this Session's agent, and take the backlog with it.
			 *
			 * Without pending backlog rows this is exactly the composer's Stop
			 * button. With them, each `queued` (turn-end) row is first promoted to
			 * next-step delivery — the native queue dock's Steer, which is only
			 * accepted while the agent is steerable — and the current step is then
			 * cancelled, so the promoted messages enter the context immediately and
			 * the turn continues with them instead of waiting for its natural end.
			 * The composer draft is never submitted by Escape.
			 */
			const runEscape = react.useCallback(async () => {
				...
				try {
					...
					for (const row of s.queue.filter((item) => item.placement === "queued")) {
						const result = await face.updateQueue(row.id, { kind: "steer" });
						if (result && result.ok === true) continue;
						// A row the agent already claimed, or an agent that stopped
						// before a command, converges silently — the same tolerance
						// the native accelerated-Enter steer has.
						const code = result?.error?.code;
						if (code !== "session/steer-unavailable" && code !== "session/queue-item-not-found") {
							console.error("queue-steer-button: escape steer rejected", result);
						}
						break;
					}
					const cancelled = await face.cancel();
