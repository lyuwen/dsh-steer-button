window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		/**
		 * Focus the composer editor belonging to the same composer stack as
		 ...
		 * The composer has been a contenteditable editor (`[data-composer-input]`)
		 * since the 0.1.5 client; the earlier `<textarea>` stays as a fallback.
		 */
		const focusComposer = (anchor) => {
			const host = anchor?.parentElement ?? document;
			const editor = host.querySelector("[data-composer-card] [data-composer-input]")
				?? host.querySelector("[data-composer-card] textarea");
		...
		/**
		 * Resolve one Session slot entry's Session/Input values.
		 *
		 * Newer DSH declares `conversation.input.right` without an owner, so the
		 * InputZone owner values (`session` / `input`) are no longer delivered to
		 * that entry and the registration only receives the slot framework's
		 * standard props — `useSession`, `useInput`, `inputActions`, `sessionId`.
		 * The hooks are therefore the primary source; the owner values remain as a
		 * fallback for compositions that still supply them.
		 */
		const slotSession = (props) => props.useSession === undefined
			? props.session
			: props.useSession((snapshot) => snapshot);
		const slotInput = (props) => props.useInput === undefined
			? props.input
			: props.useInput((snapshot) => snapshot);
		...
		const QueueSteerActions = (props) => {
			const session = slotSession(props);
			const input = slotInput(props);
			...
			// Attachments (images and files) belong to the native composer's own
			// submission: these three actions send plain text through
			// `session.prompt`, which would silently drop them. DSH replaced
			// `imageIds` with `attachmentIds`; the old name stays as a fallback.
			const attachments = input?.attachmentIds?.length ?? input?.imageIds?.length ?? 0;
			const enabled = !removed && !empty && !busy && attachments === 0 && phase === "plain";
			const queue = (session?.queue ?? []).filter((row) => row.placement === "queued" || row.placement === "steering");
			// Which Session the workspace is showing. Escape acts only for that
			// one, so a key press aimed at the visible Session can never stop a
			// background Session's turn.
			const currentSessionId = props.useSessions === undefined ? undefined : props.useSessions((state) => state.current);
			const [pending, setPending] = react.useState(null);
			const stateRef = react.useRef({});
			stateRef.current = { enabled, draft, sessionId, inputActions, pending, running, queue, currentSessionId };
			...
			/**
			 * Escape: stop this Session's agent and deliver what was pending.
			 *
			 * Without pending rows this is exactly the composer's Stop button.
			 * With them, the pending text is withdrawn from the inbox, the current
			 * step is cancelled, and the withdrawn text is re-submitted as a
			 * steering prompt — the same delivery the Steer button gives the draft,
			 * which is what actually wakes the driver.
			 *
			 * Promoting `queued` rows in place (`updateQueue(…, { kind: "steer" })`)
			 * is NOT enough: `session.cancel()` reaches `agent.cancel(…,
			 * { keepInbox: true })`, so the inbox survives, the aborted turn never
			 * reaches the step boundary that would have delivered those rows, and
			 * they stay parked with nothing to wake them. Hence: take the rows out
			 * first, then stop, then wake with their text. Rows are withdrawn only
			 * when the removal succeeded (a row the agent already claimed stays
			 * with the agent), and a row with no text projection — an
			 * attachment-only row — is left parked rather than dropped, since it
			 * cannot be re-submitted through `prompt`. The composer draft is never
			 * submitted by Escape.
			 */
			const runEscape = react.useCallback(async () => {
				const s = stateRef.current;
				if (s.pending !== null || !s.running) return;
				setPending("escape");
				try {
					const face = pluginCtx?.sessions?.binding(s.sessionId)?.session;
					if (face === undefined) {
					...
					for (const row of s.queue) {
						...
						if (result && result.ok === true) {
							withdrawn.push(row.text);
							continue;
						}
						// Already claimed by the agent, or an agent that stopped before
						// a command: both converge silently, as the native accelerated
						// steer does. Never re-submit a row we did not withdraw.
						const code = result?.error?.code;
						if (code !== "session/steer-unavailable" && code !== "session/queue-item-not-found") {
							console.error("queue-steer-button: escape withdraw rejected", result);
						}
					}
					const cancelled = await face.cancel();
					if (cancelled && cancelled.ok === false) {
						console.error("queue-steer-button: escape stop rejected", cancelled.error);
					}
					for (const text of withdrawn) {
						const result = await face.prompt([{ type: "text", text }], "steer");
						if (!(result && result.ok === true)) {
							console.error("queue-steer-button: escape delivery rejected", result);
						}
					}
				} catch (error) {
					console.error("queue-steer-button: escape failed", error);
				} finally {
					setPending(null);
				}
			}, []);
			...
			react.useEffect(() => {
				...
				// Escape: stop the agent of the Session the workspace is showing,
				// taking its pending backlog into the next step (see runEscape).
				// Deliberately bubble-phase and defaultPrevented-aware: an open
				// trigger menu, popup, inline editor, or dialog owns Escape first,
				// and a Session that is not the current one is never touched.
				const onKeyDownEscape = (e) => {
					...
					const s = stateRef.current;
					if (!s.running) return;
					if (s.currentSessionId !== undefined && s.sessionId !== s.currentSessionId) return;
					if (document.querySelector('[role="listbox"], [role="menu"], [role="dialog"]') !== null) return;
					const target = e.target;
					if (target instanceof HTMLElement
						...
						return; // a form field keeps Escape for itself
					}
					e.preventDefault();
					runEscape();
				};
				...
				document.addEventListener("keydown", onKeyDownEscape, false);
				document.addEventListener("keydown", onKeyDownBubble, false);
				return () => {
					document.removeEventListener("keydown", onKeyDownCapture, true);
					document.removeEventListener("keydown", onKeyDownEscape, false);
